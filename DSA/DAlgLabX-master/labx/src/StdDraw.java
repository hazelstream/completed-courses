 ...
 * @author Kevin Wayne Modifications by Magnus Nielsen and Oskar Holmström.
 */
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	 ...
	 * null) throw new IllegalArgumentException(); // from a file or URL try { URL
	 * url = new URL(filename); BufferedImage image = ImageIO.read(url); return
	 * image; } catch (IOException e) { // ignore } // in case file is inside a .jar
	 * (classpath relative to StdDraw) try { URL url =
	 * StdDraw.class.getResource(filename); BufferedImage image = ImageIO.read(url);
	 * return image; } catch (IOException e) { // ignore } // in case file is inside
	 * a .jar (classpath relative to root of jar) try { URL url =
	 * StdDraw.class.getResource("/" + filename); BufferedImage image =
