...
public class AVLTree<T extends Comparable<T>> {
	...
	public void remove(T key) {
		try {
			...
			System.out.println("Fat glitch");
		}

	}

//	public void simpleRemove(T key) {
//
//	}
	...
	public class AVLTreeNode<T extends Comparable<T>> {
		...
		private void calculateMinHeight(AVLTreeNode<T> n) {
			n.height = 1 + Math.min(nodeHeight(n.left), nodeHeight(n.right));
		}
		...
		private AVLTreeNode<T> remove(T x, AVLTreeNode<T> t) {
			...
			// Going left if x is less than the current node's data
			...
			// Value found
				...
				// Two children
					...
					AVLTreeNode<T> temp = t.right.findMin();
		...
		private AVLTreeNode<T> balance(AVLTreeNode<T> t) {//as all things should be

			if (nodeHeight(t.left) - nodeHeight(t.right) == -2) {
				if (nodeHeight(t.right.left) <= nodeHeight(t.right.right)) {
					return singleRotationWithRightChild(t);
				} else {
					return doubleRotationWithRightChild(t);
				}

			} else if (nodeHeight(t.right) - nodeHeight(t.left) == -2) {
				if (nodeHeight(t.left.left) >= nodeHeight(t.left.right)) {
					return singleRotationWithLeftChild(t);
				} else {
					return doubleRotationWithLeftChild(t);
				}
